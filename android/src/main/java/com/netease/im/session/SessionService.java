...
//        sendMessage(message, onSendMessageListener);
    ...
    //@newedit 发送商品链接
    public void sendProductLink(String custype, String id,String name,String price,String pic_url, OnSendMessageListener onSendMessageListener) {
        CustomMessageConfig config = new CustomMessageConfig();
        LinkUrlAttachment attachment = new LinkUrlAttachment();
        attachment.setParams(name, price, pic_url,id);
        String data= attachment.toJson(true);
        IMMessage message = MessageBuilder.createCustomMessage(sessionId, sessionTypeEnum, data,attachment,config);

        LogUtil.w(TAG, "product=-=========");
        sendMessage(message, onSendMessageListener);
    }
