...
-(void)sendTipMessage:( NSString *)content{
    NIMMessage *tipMessage = [self msgWithTip:content];
    //构造会话
    NIMSession *session = [NIMSession session:_session.sessionId type:NIMSessionTypeP2P];
    
    [[NIMSDK sharedSDK].chatManager sendMessage:tipMessage toSession:session error:nil];
}
