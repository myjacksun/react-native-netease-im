...
//发送自定义消息2 @newedit
RCT_EXPORT_METHOD(sendCustomMessage:(nonnull NSString *)custype data:(NSString *)dataDict){
      NSInteger type =7;
      NSDictionary *d = @{@"product": @"111"};
    [[ConversationViewController initWithConversationViewController]sendCustomMessage:&type data:d];
}

//发送产品链接 @newedit
RCT_EXPORT_METHOD(sendProductLink:(NSString *)custype :(NSString *)id :(NSString *)name :(NSString *)price :(NSString *)pic_url){
    NSInteger type =7;
    NSDictionary *dataDict = @{@"image": pic_url,@"title": name,@"describe": price,@"linkUrl": id};
    [[ConversationViewController initWithConversationViewController]sendCustomMessage:&type data:dataDict];
}
